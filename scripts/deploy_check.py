...
ok = lambda s: f"{_GREEN}✓{_RESET} {s}"
fail = lambda s: f"{_RED}✗{_RESET} {s}"
warn = lambda s: f"{_YELLOW}~{_RESET} {s}"
    ...
    try:
        ...
        async def _ping():
            async with AsyncSessionLocal() as session:
                result = await session.execute(text("SELECT COUNT(*) FROM chunks WHERE is_deleted = FALSE"))
